  ...
  if(!init_ || !start_tracking_){
    ...
    if(q.total()==0){
      ...
      for(int i =0; i<clustersize; ++i){
        ...
        const Matrices& association_matrices = generate_hypothesis(x);
        //std::cout<<"########## association_matrices ##########\n"<<std::endl;

        Eigen::MatrixXd beta = joint_probability(association_matrices, selectdets, tracks);
        //std::cout<<"########## BETA part ##########\n"<<beta<<std::endl;
        int b=0;
    ...
    pruning(selected_detections,final_select,prun_tracks);
    ...
    if(unconfirmed_tracks_.size()>0){
      ...
      const uint& UconfirmTrackSize = unconfirmed_tracks_.size();
      const uint& detSize = seconda_sso_detections.size();
...
void Tracker::associate(Detection& _selected_detections, cv::Mat& _q, const Detection& _detections)
{
  ...
  for(const auto& detection : _detections){
    ...
    if(found){
      _selected_detections.push_back(detection);
      validationIdx++;
    }else{
